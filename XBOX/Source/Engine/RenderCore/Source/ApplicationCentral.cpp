...
bool ApplicationCentral::CreateFileDatabases(ApplicationModule *pModule)
{
  ...
  String databaseNames[] = { "Model", "Texture", "Material", "UI", "Import" };
  for (uint i = 0; i < 5; i++)
  {
    ...
    switch (i)
    {
      ...
      case 4: m_pImportDatabase   = FileSystem.AddDatabase(databaseNames[i], inDir); break;
    }
  }

  if (m_pModelDatabase && m_pTextureDatabase && m_pMaterialDatabase && m_pUIDatabase && m_pImportDatabase)
    return true;
