...
{
  center.Add(*((Vec3*)(&(matrix._41))));
  //center.TransformCoord(matrix);

  float scaleXSq = (matrix._11 * matrix._11) + (matrix._12 * matrix._12) + (matrix._13 * matrix._13);
  float scaleYSq = (matrix._21 * matrix._21) + (matrix._22 * matrix._22) + (matrix._23 * matrix._23);
  float scaleZSq = (matrix._31 * matrix._31) + (matrix._32 * matrix._32) + (matrix._33 * matrix._33);

  float scaleSq  = Max<float>(Max<float>(scaleXSq, scaleYSq), scaleZSq);

  if (scaleSq == 1.0f || scaleSq == 0.0f)
    return;

  radius *= sqrtf(scaleSq);
}
...
// ----------------------------------------------------------------------------
// Name: Sphere::TestCollision
// Desc: Tests if the line segment interscts the sphere
// Parm: const LineSegment &lineSegment - the line segment
// ----------------------------------------------------------------------------
...
{
#if XBOX
  return (D3DXSphereBoundProbe(&center, radius, const_cast<Vec3*>(&ray.position), const_cast<Vec3*>(&ray.direction)) == TRUE);
#else
