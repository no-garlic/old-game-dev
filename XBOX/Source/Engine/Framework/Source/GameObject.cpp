...
: Parent()
{
  memset(m_pInterfaces, 0, GOI_MAX * sizeof(GameObjectInterface *));
...
GameObject::~GameObject()
{
  for (uint i = 0; i < GOI_MAX; i++)
  {
    delete m_pInterfaces[i];
  }
}
...
void GameObject::Update(float deltaTime)
{
  for (uint i = 0; i < GOI_MAX; i++)
  {
    if (m_pInterfaces[i])
    {
      m_pInterfaces[i]->Update(deltaTime);
    }
  }
}
...
void GameObject::UpdateWorldTransform()
{
  ...
  IRender *pRenderInterface = GetInterface<IRender>();
  if (pRenderInterface)
  {
    pRenderInterface->SetBoundsDirty();
  }
}
