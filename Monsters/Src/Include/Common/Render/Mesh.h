...
public:
  ...
  // Retn: D3DMATERIAL9 & - the material
  // --------------------------------------------------------------------------
  inline D3DMATERIAL9 &GetColorMaterial();
  inline const D3DMATERIAL9 &GetColorMaterial() const;
  ...
  struct FileHeader
  {
    D3DMATERIAL9    ColorMaterial;    // The color material
    uint            FaceStart;        // The first face to render
    uint            FaceCount;        // The number of faces to render
    FileString<256> DiffuseTexture;   // The diffuse texture
    FileString<256> NormalTexture;    // The normal texture
    FileString<256> AlphaTexture;     // The alpha texture
    FileString<256> Tex0;             // Extra texture slot 0
    FileString<256> Tex1;             // Extra texture slot 1
    FileString<256> EffectFilename;   // The effect filename
  }; // FileHeader
...
private:
  ...
  // Private members
  // --------------------------------------------------------------------------
  D3DMATERIAL9    m_ColorMaterial;    // The color material
  ...
  Material       *m_pMaterial;        // The material (effect) to use
...
public:
  ...
  // --------------------------------------------------------------------------
  virtual void Render(uint pass = 0) = 0;
  ...
  // Parm: uint pass    - the pass
  // --------------------------------------------------------------------------
  virtual void RenderSubset(uint subset, uint pass = 0) = 0;
...
// ----------------------------------------------------------------------------
inline uint Mesh::GetNumSubsets()
{
...
// --------------------------------------------------------------------------
inline D3DMATERIAL9 &MeshSubset::GetColorMaterial()
{
...
// Get the color material
// --------------------------------------------------------------------------
inline const D3DMATERIAL9 &MeshSubset::GetColorMaterial() const
{
  return m_ColorMaterial;
} // GetColorMaterial
...
// Get the material
// ----------------------------------------------------------------------------
inline Material *MeshSubset::GetMaterial()
{
  return m_pMaterial;
} // GetMaterial
...
// Set the material
// ----------------------------------------------------------------------------
inline void MeshSubset::SetMaterial(Material *pMaterial)
{
