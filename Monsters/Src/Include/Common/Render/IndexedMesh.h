...
public:
  ...
  // --------------------------------------------------------------------------
  virtual void Render(uint pass = 0);
  ...
  // Parm: uint pass    - the pass
  // --------------------------------------------------------------------------
  virtual void RenderSubset(uint subset, uint pass = 0);
  ...
  // --------------------------------------------------------------------------
  // Name: BuildVertexType
  // Desc: Build the vertex type from the vertex declaration associated with a 
  //       mesh
  // Parm: D3DVERTEXELEMENT9 *pDeclaration - the declaration to build from
  // PArm: const String &name              - the vertex name
  // Retn: bool                            - true if a valid vertex type was
  //                                         created
  // --------------------------------------------------------------------------
  bool BuildVertexType(const String &name, D3DVERTEXELEMENT9 *pDeclaration);
  ...
  struct FileHeader
  {
    ...
    uint                  NumDeclaration;                 // The number of declaration items
    D3DVERTEXELEMENT9     Declaration[MAX_FVF_DECL_SIZE]; // The declaration
...
private:
  ...
  IDirect3DVertexBuffer9   *m_pvertex_buffer;         // The direct 3D vertex buffer
  IDirect3DIndexBuffer9    *m_pindex_buffer;          // The direct 3D index buffer
  ...
  bool                      m_vertex_dirty;           // Is the D3D vertex data dirty
  bool                      m_index_dirty;            // Is the D3D index data dirty
  bool                      m_rebuild_vertex_array;   // Does the D3D vertex buffer need to be recreated
  bool                      m_rebuild_index_array;    // Does the D3D index buffer need to be recreated
