...
// before the render.
// --------------------------------------------------------------------------
void Mesh::SetPreRenderCallback(const Callback<void, MeshSubset *> &cb)
{
  delete m_pPreRenderCB;
  m_pPreRenderCB = cb.Clone();
}
...
// Desc: Constructor
// ----------------------------------------------------------------------------
MeshSubset::MeshSubset(uint index)
: m_pMaterial   (NULL),
  m_Index       (index),
  m_FaceCount   (0),
  m_FaceStart   (0),
  ...
  m_VertexCount (0)
{
  ZeroMemory(&m_ColorMaterial, sizeof(D3DMATERIAL));
} // MeshSubset
...
void Mesh::PrintSubsets()
{
  for (uint i = 0; i < GetNumSubsets(); i++)
  {
    ...
    LOG_INFO << "Subset: " 
      << subset.GetFaceStart() << ", " 
      << subset.GetFaceCount() << ", " 
      << (subset.GetMaterial() ? subset.GetMaterial()->GetName() : "NULL") << ", " 
      << (subset.GetTextureArray().GetTex0() ? subset.GetTextureArray().GetTex0()->GetFileHandle().GetFilename() : "NULL") 
      << ENDLOG;
  } // for
} // PrintSubsets
