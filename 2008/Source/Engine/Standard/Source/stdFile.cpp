...
// Parm: FileDatabase *pFileDatabase - the database
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// Name: FileHandle::IsValid
// Desc: Check if the file handle is valid (checks that it has a database and 
//       filename - does not check if the file exists or not.
// Retn: bool - true if it is
// ----------------------------------------------------------------------------
...
// Returns true if the dependency is ok (ie, up-to-date)
...
// ----------------------------------------------------------------------------
// Name: FindFileInPath
// Desc: Searches recursively for a given file
// Parm: const String &basePath   - the path to start looking from
// Parm: const String &filename   - the filename to locate
// Parm: String &absoluteFilename - the absolute filename of the file (if found)
// Parm: String &relativePath     - the path from the base path where the file
//                                  was found (abs = base + rel + filename).
// Retn: true if found, otherwise false. If found, both absoluteFilename and 
//       relativePath will be set.
// ----------------------------------------------------------------------------
    ...
    do
    {
      ...
      else
      {
        if (_stricmp(filename.c_str(), wfd.cFileName) == 0)
        {
          ...
          OutputDebugString(absoluteFilename.c_str());
          OutputDebugString("\n");
          return true;
        }
      }
...
// ----------------------------------------------------------------------------
// Name: FileDatabase::FindFileHandle
// Desc: Make a new file handle for an existing file by looking recursively 
//       through all paths in the file database.
// Parm: const String &filename - the filename
// Retn: FileHandle             - the handle
// ----------------------------------------------------------------------------
