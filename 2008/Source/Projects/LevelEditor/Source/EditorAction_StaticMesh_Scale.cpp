...
EditorAction_StaticMesh_Scale::EditorAction_StaticMesh_Scale()
: EditorAction  (),
  m_pNode       (NULL)
{
}
...
bool EditorAction_StaticMesh_Scale::Init(const String &nodePath)
{
...
bool EditorAction_StaticMesh_Scale::Complete()
{
...
void EditorAction_StaticMesh_Scale::Discard()
{
  NULL_ASSERT(m_pNode);
...
void EditorAction_StaticMesh_Scale::Update()
{
  NULL_ASSERT(m_pNode);
  ...
  if (m_Plane.Intersect(true, ray, hitPt))
  {
    ...
    Vec3 gridScale(m_Scale.x + delta.x, m_Scale.y + delta.y, m_Scale.z + delta.z);
    Editor.GetGrid()->Snap(0, 0, &gridScale);
    m_pNode->SetScale(gridScale.x, gridScale.y, gridScale.z);
  }
